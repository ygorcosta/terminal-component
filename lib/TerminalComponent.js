...
class TerminalComponent extends Component {
  ...
  type() {
    ...
    demoConsole.animate([{ opacity: 0, transform: `${demoConsoleTransform} translateX(20px)` }, { opacity: 1, transform: `${demoConsoleTransform} translateX(0)` }], {
      delay: 300,
      duration: 600,
      easing: 'ease-in-out',
      fill: 'backwards'
  ...
  console() {
    ...
    runInput('ls', { speed: 100, beforeMs: 1000, afterMs: 1000 }).then(() => runHtml('\n     <b>index.html</b>\n', { afterMs: 300 })).then(() => runInput('we deploy', { speed: 60, afterMs: 1500 })).then(() => runHtml('\n  >  <b>yourapp</b> in <b>wedeploy</b>', { afterMs: 100 })).then(() => runHtml('\n     Initializing deployment process\n     Preparing package', { afterMs: 1500 })).then(() => runHtml('\n  <b>!</b>  Deployment Successful in 3s', { afterMs: 200 })).then(() => runHtml('\n  <b>!</b>  Deployed <b>hosting-yourapp.wedeploy.io</b>\n', { afterMs: 100 })).then(() => runInput('', { isInteractive: true }));
    ...
    function runHtml(content, rawopts) {
      ...
      return new Promise(resolve => setTimeout(resolve, opts.beforeMs || 0)).then(() => pushHtml(content)).then(() => new Promise(resolve => setTimeout(resolve, opts.afterMs || 0)));
    }
    ...
    function runWeThenInput(args) {
      ...
      const cmdDocsByIndex = [null, null, 'login', 'logout', 'deploy', 'log', 'domain', 'env', 'list', 'delete', 'console', 'update'];
